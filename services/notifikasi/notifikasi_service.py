...
from bank_djago.penyimpanan.sqlite.database import buat_koneksi, buat_koneksi_baca, buat_koneksi_tulis
...
class NotifikasiService:
    ...
    def _normalisasi_data_notifikasi(daftar_notifikasi):
        ...
        for data_notif in daftar_notifikasi:
            ...
            data_notif['jenis_referensi'] = JenisReferensi.dari_nilai(data_notif['jenis_referensi'])
    ...
    def cari_semua_notifikasi_belum_dibaca(nik):
        with buat_koneksi_baca() as koneksi:
          daftar_notifikasi =  NotifikasiRepository.cari_notifikasi_belum_dibaca(nik=nik, koneksi=koneksi)

        return NotifikasiService._normalisasi_data_notifikasi(daftar_notifikasi=daftar_notifikasi)
        ...
        for data_notif in daftar_notifikasi:
            try:
                with buat_koneksi_tulis() as koneksi:
                    NotifikasiRepository.tandai_sudah_dibaca(id_notifikasi=data_notif['id'], koneksi=koneksi)
                    jumlah_berhasil += 1
            except Exception:
                jumlah_gagal += 1
                continue
