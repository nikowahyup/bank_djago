...
class NasabahService:
    ...
    def daftar_dan_buka_rekening(nik, nama, alamat, pin, setor_awal, level):
        ...
        with buat_koneksi_tulis() as koneksi:
            validasi_nik = NasabahRepository.cari_nasabah_dengan_nik(
    ...
    @staticmethod
    def ganti_alamat(nik, alamat_baru):

        alamat_baru = alamat_baru.strip()

        if not alamat_baru:
            raise InputTidakValid("Alamat baru tidak boleh kosong")
        ...
        with buat_koneksi_tulis() as koneksi:
            ...
            if data_nasabah is None:
                raise NasabahTidakDitemukan("Nasabah tidak terdaftar")

            nasabah = NasabahLoader.rangkai_nasabah(data_nasabah=data_nasabah)
            alamat_lama = nasabah.alamat

            if nasabah.alamat == alamat_baru:
                raise InputTidakValid("Alamat baru tidak boleh sama dengan alamat lama")
            ...
            if jumlah_baris != 1:
                raise PerbaruiStatusGagal("Gagal memperbarui alamat nasabah")
    ...
    def cari_data_login(nik):
        ...
        return {"nik": data_nasabah["nik"], "nama": data_nasabah["nama"]}
