...
class DepositoService:
    ...
    JANGKA_WAKTU = {1: 0.03, 3: 0.035, 6: 0.04, 12: 0.045}
    MIN_DEPO = 1_000_000
    ...
    def buka_deposito(
        ...
        if nominal < DepositoService.MIN_DEPO:
            raise InputTidakValid("Nominal deposito tidak memenuhi syarat")

        if lama_bulan not in DepositoService.JANGKA_WAKTU:
            raise InputTidakValid("Jangka waktu deposito tidak tersedia")
        ...
        if jenis_aro == JenisAro.TIDAK:
            ...
            if lama_aro not in DepositoService.JANGKA_WAKTU:
                raise JenisAroTidakValid("Jangka waktu perpanjangan tidak tersedia")

        else:
            raise JenisAroTidakValid("Jenis ARO tidak tersedia")
        ...
        with buat_koneksi_tulis() as koneksi:
            ...
            if rekening.saldo - nominal < rekening.saldosetor_min:
                raise StatusTidakValid(
                    f"Saldo rekening akan kurang dari minimal saldo\n"
                    f" mengendap jika membuka deposito Rp{Utilitas.format_rupiah(nominal)}"
                )
            ...
            jatuh_tempo = Utilitas.tambah_bulan(tanggal=tanggal_buka, bulan=lama_bulan)
            ...
            id_deposito = DepositoRepository.tambah_deposito(
                deposito=deposito_baru, koneksi=koneksi
            )
    ...
    @staticmethod
    def cairkan_deposito(nik, norek_pencairan, id_deposito, hari_ini=None):

        if hari_ini is None:
            hari_ini = datetime.date.today()

        if not isinstance(id_deposito, int):
            raise InputTidakValid("ID deposito harus berupa angka")

        if id_deposito <= 0:
            raise InputTidakValid("ID deposito tidak valid")
        ...
        with buat_koneksi_tulis() as koneksi:
            ...
            if data_deposito is None:
                raise DepositoTidakDitemukan("Deposito tidak ditemukan")
            ...
            if nasabah.NIK != nik:
                raise NikTidakSesuai("NIK ini tidak terdaftar sebagai pemilik deposito")
            Validator.amankan_rekening(rekening=rekening)

            deposito = DepositoLoader.rangkai_deposito(
                data_deposito=data_deposito, nasabah=nasabah, rekening=rekening
            )
            ...
            if hari_ini < deposito.jatuh_tempo:
                raise StatusTidakValid("Deposito belum jatuh tempo")
    ...
    @staticmethod
    def perpanjangan(deposito, koneksi, hari_ini=None):

        if hari_ini is None:
            hari_ini = datetime.date.today()

        data_deposito = DepositoRepository.cari_deposito_dengan_id(
            id_deposito=deposito.ID, koneksi=koneksi
        )

        if data_deposito is None:
            raise DepositoTidakDitemukan("Data deposito tidak ditemukan")
        norek = data_deposito["norek"]
        ...
        if jumlah_baris_deposito != 1:
            raise PerbaruiStatusGagal("Terjadi kesalahan saat memperbarui ARO")
        ...
        if deposito.jenis_aro == JenisAro.POKOK:
            ...
            if jumlah_baris_rekening != 1:
                raise PenambahanSaldoGagal("Gagal menambahkan bunga ke rekening")
            saldo_baru = RekeningRepository.ambil_saldo(norek=norek, koneksi=koneksi)
        ...
        AuditRepository.tambah_audit(
            audit=audit_aro, koneksi=koneksi, id_transaksi=id_transaksi
        )
    ...
    @staticmethod
    def hentikan_aro(nik, norek_pemberhentian, id_deposito):
        ...
        with buat_koneksi_tulis() as koneksi:
            ...
            if deposito.status != StatusDeposito.AKTIF:
                raise StatusTidakValid("ARO hanya dapat dihentikan pada deposito aktif")

            if deposito.jenis_aro == JenisAro.TIDAK:
                raise JenisAroTidakValid("Deposito ini tidak menggunakan ARO")

            if deposito.jenis_aro not in (JenisAro.POKOK, JenisAro.POKOK_BUNGA):
                raise JenisAroTidakValid("Jenis ARO deposito tidak valid")
            ...
            if jumlah_baris != 1:
                raise PerbaruiStatusGagal("Gagal memperbarui pengaturan ARO")
    ...
    def tandai_jatuh_tempo(deposito, koneksi, hari_ini=None):
        ...
        if data_deposito["jenis_aro"] != JenisAro.TIDAK:
            raise JenisAroTidakValid("Deposito ARO harus diproses di perpanjangan")
        ...
        if jumlah_baris != 1:
            raise PerbaruiStatusGagal("Gagal memperbarui status deposito")
    ...
    def _normalisasi_data_deposito(data_deposito):
        ...
        data["tanggal_buka"] = datetime.date.fromisoformat(data["tanggal_buka"])

        data["jatuh_tempo"] = datetime.date.fromisoformat(data["jatuh_tempo"])

        if data["proses_aro"] is not None:
            data["proses_aro"] = datetime.date.fromisoformat(data["proses_aro"])

        data["total_pencairan"] = Deposito.hitung_total_pencairan(
            nominal=data["nominal"], bunga=data["bunga"], lama_bulan=data["lama_bulan"]
        )
    ...
    def cari_deposito_nasabah(nik):
        daftar_deposito = DepositoRepository.cari_deposito_dengan_nik(nik=nik)
    ...
    def cari_deposito_jatuh_tempo_dengan_norek(norek):
        ...
        daftar_deposito = DepositoRepository.cari_deposito_jatuh_tempo_dengan_norek(
            norek=norek
        )
    ...
    def cari_deposito_aro_aktif_dengan_norek(norek):
        ...
        daftar_deposito = DepositoRepository.cari_deposito_aro_aktif_dengan_norek(
            norek=norek
        )
