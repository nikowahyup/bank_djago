...
class DepositoLoader:
    ...
    def muat_deposito(nasabah):
        ...
        for data in daftar_deposito:
            ...
            if rekening is None:
                raise ValueError(
                    f"Rekening untuk deposito {data["id"]} tidak ditemukan"
                )
    ...
    def muat_semua_deposito_aktif():
        ...
        with buat_koneksi_tulis() as koneksi:
            data_deposito = DepositoRepository.cari_semua_deposito_aktif(
                koneksi=koneksi
            )
            for data_deposito in data_deposito:
                ...
                if norek in rekening_index:
                    ...
                    if data_rekening is None:
                        raise ValueError(
                            f"Rekening untuk deposito ber-ID {data_deposito['id']} tidak ditemukan"
                        )
                    ...
                    if nik in nasabah_index:
                        ...
                        if data_nasabah is None:
                            raise ValueError(
                                f"Nasabah untuk deposito ber-ID {data_deposito['id']} tidak ditemukan"
                            )
                ...
                deposito = Deposito(
                    ...
                    tanggal_buka=datetime.date.fromisoformat(
                        data_deposito["tanggal_buka"]
                    ),
    ...
    def rangkai_deposito(data_deposito, nasabah, rekening):
        ...
        proses_aro = data_deposito["proses_aro"]
        deposito.proses_aro = (
            datetime.date.fromisoformat(proses_aro) if proses_aro is not None else None
        )
