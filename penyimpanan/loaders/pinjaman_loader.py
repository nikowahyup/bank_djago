...
class PinjamanLoader:
    ...
    def muat_pinjaman(nasabah):
        ...
        for data_pinjaman in daftar_pinjaman:
            ...
            if rekening is None:
                raise ValueError(
                    f"Rekening untuk pinjaman ber-ID {data_pinjaman['id']} tidak ditemukan"
                )
    ...
    def muat_semua_pinjaman_aktif():
        ...
        with buat_koneksi_tulis() as koneksi:
            daftar_pinjaman = PinjamanRepository.cari_semua_pinjaman_aktif(
                koneksi=koneksi
            )
            ...
            for data_pinjaman in daftar_pinjaman:
                ...
                if nik not in nasabah_index:
                    ...
                    if data_nasabah is None:
                        raise ValueError(
                            f"Nasabah untuk pinjaman {data_pinjaman['id']} tidak ditemukan"
                        )
                ...
                if norek not in rekening_index:
                    ...
                    if data_rekening is None:
                        raise ValueError(
                            f"Rekening untuk pinjaman {data_pinjaman['id']} tidak ditemukan"
                        )

                    rekening_index[norek] = RekeningLoader.rangkai_rekening(
                        data_rekening, nasabah
                    )
