...
class RekapRepository:
    ...
    # method untuk mencari data dasar
    @staticmethod
    def rekap_umum():
        koneksi = buat_koneksi()

        try:
            hasil_nasabah = koneksi.execute("""SELECT COUNT (*) 
            AS total_nasabah 
            FROM nasabah
            """).fetchone()
            ...
            hasil_rekening = koneksi.execute("""SELECT COUNT (*)
             AS total_rekening,
    ...
    # method untuk melihat tota tiap jenis rekening
    @staticmethod
    def jumlah_tiap_jenis_rekening():
        koneksi = buat_koneksi()

        try:
            jumlah_rekening = koneksi.execute("""SELECT
                level,
    ...
    # method untuk melihat tiap status rekening
    @staticmethod
    def rekap_status_rekening():
        ...
        try:
            status_rekening = koneksi.execute("""SELECT 
                    status,
                    COUNT (*) AS total_rekening 
                    FROM rekening
                    GROUP BY status
    ...
    # method untuk melihat total saldo tiap jenis rekening
    @staticmethod
    def total_saldo_tiap_jenis_rekening():
        ...
        try:
            jumlah_saldo = koneksi.execute("""SELECT
                    level, 
    ...
    def cari_saldo_terbesar():
        ...
        try:
            saldo_terbesar = koneksi.execute("""SELECT
    ...
    def cari_saldo_terkecil():
        ...
        try:
            saldo_terkecil = koneksi.execute("""SELECT
                    nasabah.nama AS nama,
                    rekening.saldo AS saldo
                    FROM rekening
                    JOIN nasabah
